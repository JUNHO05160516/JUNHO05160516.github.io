...
3) 정형 데이터: 정형화된 구조, 연산 가능 (예: 엑셀, 스프레드시트, 관계형 데이터베이스 (DBMS), Oracle, MS-SQL)
- 스키마 구조 / 고정필드 (속성) / DBMS에 저장

4) 반정형 데이터: 정형화된 구조, 연산 불가능, 데이터는 존재 (예: XML, JSON, HTML)
- 고정필드 X / 메타데이터 or 스키마 정보 포함

5) 비정형 데이터: 정형화된 구조 없음, 연산불가능, 데이터는 존재 (예: 텍스트, 문서, 이진 파일, 이미지, 동영상 등)
- 고정필드 X / 메타 데이터 X / 스키마 X
- 데이터 각각이 객체로 구분됨
...
- <u> KB < MB < GB < TB < PB < EB < ZB < YB </u>

- 킬로 < 메가 < 기가 < 테라 < 페타 < 에타 < 제타 < 요타
...
#### 2. 지식경영
1) 암묵지
- 학습과 경험을 통해 개인에게 축적된 <u>내면화된 지식</u> (예: 공부)
- 개인에게 습득되어 있지만 겉으로 드러나지 않은 지식
- 오랜 경험을 통해 개인에게 습득된 무형의 지식
- 조직의 지식으로 <u>공통화</u> (예: 인수인계)
- 외부에 표출되어 다른 사람에게 고유되기 어려움

2) 형식지
- 문서나 메뉴얼처럼 <u>형상화된 지식</u>
- 언어, 기호, 숫자로 <u>표출화된 지식</u> (예: 논문 퍼블리시)
- 개인의 지식으로 <u>연결화</u> (예: 후속 연구)
- 전달과 공유가 용이하다.
...
2) 정보 (**I**nformation)
- 데이터를 가공 및 처리 -> <u>연관관계 & 의미</u>가 도출된 데이터
- 데이터 간의 관계 및 현상의 분석

3) 지식 (**K**nowledge)
- 정보를 구조화 -> <u>분류 & 일반화</u>시킨 결과물 또는 규칙
